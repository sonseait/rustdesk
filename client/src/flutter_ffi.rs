...
/// Returns managed enrollment state without exposing private key material.
pub fn managed_get_status() -> SyncReturn<String> {
    SyncReturn(serde_json::to_string(&crate::managed_client::status()).unwrap_or_else(|_| "{}".to_owned()))
}

pub fn managed_capability_allowed(capability: String) -> bool {
    crate::managed_client::capability_allowed(&capability)
}

/// Stores only the control-plane URL in normal client configuration.
pub fn managed_configure(control_plane_url: String) -> String {
    crate::managed_client::configure(control_plane_url).err().unwrap_or_default()
}

/// Enrolls this device. The enrollment token and generated signing key never
/// cross back to the Flutter presentation layer.
pub fn managed_enroll(enrollment_token: String, display_name: String) -> String {
    crate::managed_client::enroll(enrollment_token, display_name)
...
pub fn managed_renew_credential() -> String {
    crate::managed_client::renew_credential().err().unwrap_or_default()
}

/// Ticket material stays in Rust until the managed wire protocol carries it.
pub fn managed_request_session_ticket(target_device_id: String) -> String {
    crate::managed_client::request_session_ticket(target_device_id).err().unwrap_or_default()
}

pub fn managed_request_session_ticket_for_rustdesk_id(target_rustdesk_id: String) -> String {
    crate::managed_client::request_session_ticket_for_rustdesk_id(target_rustdesk_id).err().unwrap_or_default()
}

pub fn managed_deprovision() -> String {
    crate::managed_client::deprovision(false).err().unwrap_or_default()
}
pub enum EventToUI {
    Event(String),
    Rgba(usize),
