...
impl RendezvousServer {
    ...
    pub async fn start_with_bind(
        ...
        crate::managed_observability::start_if_configured("HBBS_INTERNAL_METRICS_ADDR", "hbbs");
        ...
        if let Ok(msg_in) = RendezvousMessage::parse_from_bytes(bytes) {
            match msg_in.union {
                Some(rendezvous_message::Union::RegisterPeer(rp)) => {
                    // B registered
                    if !rp.id.is_empty() {
                        let managed_device_id = match crate::managed::authorize_registration(&rp.managed_credential) {
                            Ok(device_id) => device_id,
                            Err(error) => {
                                log::warn!("Managed registration rejected for peer {} from {}: {}", rp.id, addr, error);
                                return Ok(());
                            }
                        };
                        log::trace!("New peer registered: {:?} {:?}", &rp.id, &addr);
                        let managed_device_id = if let Some((device_id, credential_rustdesk_id)) = managed_device_id {
                            if credential_rustdesk_id != rp.id {
                                log::warn!("Managed registration rejected for peer {} from {}: credential identity mismatch", rp.id, addr);
                                return Ok(());
                            }
                            Some(device_id)
                        } else {
                        ...
                        // A later legacy registration must not inherit a
                        // previous device's managed identity for this ID.
                        self.pm.get_or(&rp.id).await.write().await.managed_device_id = managed_device_id;
    ...
    async fn handle_punch_hole_request(
        ...
        let id = ph.id;
        let target_device_id = if let Some(peer) = self.pm.get_in_memory(&id).await {
            peer.read().await.managed_device_id
        } else {
            None
        };
        if let Err(error) = crate::managed::authorize_session(&ph.managed_session_ticket, target_device_id) {
            log::warn!("Managed session authorization rejected for target {} from {}: {}", id, addr, error);
            let mut msg_out = RendezvousMessage::new();
            msg_out.set_punch_hole_response(PunchHoleResponse {
