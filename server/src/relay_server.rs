...
static ref PEERS: Mutex<HashMap<String, Box<dyn StreamTrait>>> = Default::default();
    // A managed source may arrive before or after its target. Remember only
    // the opaque relay UUID until that pair is consumed; no ticket is retained.
    static ref AUTHORIZED_RELAY_UUIDS: Mutex<HashSet<String>> = Default::default();
    ...
    if let Ok(Some(Ok(bytes))) = timeout(30_000, stream.recv()).await {
        if let Ok(msg_in) = RendezvousMessage::parse_from_bytes(&bytes) {
            if let Some(rendezvous_message::Union::RequestRelay(rf)) = msg_in.union {
                ...
                let managed_authorized = match crate::managed::authorize_relay(&rf.managed_session_ticket) {
                    Ok(authorized) => authorized,
                    Err(error) => {
                        log::warn!("Managed relay authorization rejected from {}: {}", addr, error);
                        return;
                    }
                };
                if !rf.uuid.is_empty() {
                    if managed_authorized {
                        AUTHORIZED_RELAY_UUIDS.lock().await.insert(rf.uuid.clone());
                    }
                    let mut peer = PEERS.lock().await.remove(&rf.uuid);
                    if let Some(peer) = peer.as_mut() {
                        let authorized = AUTHORIZED_RELAY_UUIDS.lock().await.remove(&rf.uuid);
                        if crate::managed::relay_authorization_required() && !authorized {
                            log::warn!("Managed relay pairing rejected from {} - ticket missing", addr);
                            return;
                        }
                        ...
                        AUTHORIZED_RELAY_UUIDS.lock().await.remove(&rf.uuid);
                    }
                }
            }
        }
