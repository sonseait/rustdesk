...
| `MANAGED_MODE` 🅴 | *(none)* | `off` | Managed protocol enforcement: `off` preserves legacy behavior, `optional` accepts legacy registrations but verifies supplied managed credentials, and `required` rejects uncredentialed registrations and session requests. |
| `MANAGED_CREDENTIAL_ISSUER_PUBLIC_KEY` 🅴 | *(none)* | *(required outside `off`)* | The 32-byte Ed25519 control-plane issuer public key, encoded as 64 lowercase/uppercase hexadecimal characters. Invalid or missing values fail managed authorization closed. |
...
Set the same values for both `hbbs` and `hbbr` through their `.env` file or
runtime arguments:

| Name | Values | Purpose |
| --- | --- | --- |
| `MANAGED_MODE` | `off`, `optional`, `required` | Enables managed registration and session enforcement. |
| `MANAGED_CREDENTIAL_ISSUER_PUBLIC_KEY` | 64 hexadecimal characters | Ed25519 public key used to verify device credentials and short-lived session tickets. |

`required` rejects uncredentialed registrations, punch requests, and relay
pairing. `hbbr` verifies a relay ticket independently and stores only its
opaque pairing UUID while it waits for the other stream; it never receives a
control-plane signing key or retains ticket contents.
